...
export function SubmissionHistoryPanel({ problemId, onClose }: { problemId: string; onClose: () => void }) {
  return <div className="fixed inset-0 z-[60] flex items-center justify-center bg-foreground/40 p-4" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="max-h-[85vh] w-full max-w-3xl overflow-hidden rounded-xl border border-border bg-card shadow-xl" role="dialog" aria-modal="true" aria-labelledby="submission-history-title" onKeyDown={(event) => containDialogFocus(event, onClose)}><div className="flex items-center justify-between border-b border-border p-5"><div><h2 id="submission-history-title" className="font-semibold">Histórico do exercício</h2><p className="mt-1 text-sm text-muted-foreground">Submissões anteriores deste exercício</p></div><button autoFocus onClick={onClose} aria-label="Fechar histórico" className="flex size-11 items-center justify-center rounded-md hover:bg-muted md:size-8"><X className="size-5" /></button></div><div className="max-h-[calc(85vh-88px)] overflow-y-auto p-5"><SubmissionHistoryContent problems={[]} fixedProblemId={problemId} compact /></div></div></div>
}
...
function SubmissionDetailModal({ submission, onClose }: { submission: SubmissionDetail; onClose: () => void }) {
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-foreground/40 p-4" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="flex max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden rounded-xl border border-border bg-card shadow-xl" role="dialog" aria-modal="true" aria-labelledby="submission-detail-title" onKeyDown={(event) => containDialogFocus(event, onClose)}><div className="flex items-center justify-between border-b border-border p-5"><div><h2 id="submission-detail-title" className="font-semibold">{submission.problemTitle}</h2><p className="mt-1 text-sm text-muted-foreground">{statusLabels[submission.status]} · {submission.testsPassed} / {submission.totalTests} testes · {formatExecutionTime(submission.executionTimeMs)} · {formatDate(submission.submittedAt)}</p></div><button autoFocus onClick={onClose} aria-label="Fechar solução" className="flex size-11 items-center justify-center rounded-md hover:bg-muted md:size-8"><X className="size-5" /></button></div><pre className="min-h-64 overflow-auto bg-[#1e1e1e] p-5 font-mono text-sm leading-6 text-[#d4d4d4]"><code>{submission.code}</code></pre></div></div>
}
